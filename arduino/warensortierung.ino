...
unsigned int NR_BOXES = 2;       // number of boxes
...
const int TCP_SERVER_PORT = PORT;          // see communication.h
 ...
 * @return -1 if triggered and 0 if not triggered (???)
 ...
 Direct exitFinction if barrier is triggered???
 */
int light_barrier() {
 ...
 * The resulting string is 6 Bite long 
 *
...
char* assembleData(char message, float weight) {
  ...
  //allocats memory for the String
  char* result = (char*)malloc(6 * sizeof(char));
  // case if the wight ist less then 3 digits long
  if (weight_int >= 100) {
